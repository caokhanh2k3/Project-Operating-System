...
OpenFile *FileSystem::AssignFileDescriptor(int index, char *filename, int fileType, int flagsocket) {
    if (openingFile[index] == NULL) {
        if(flagsocket != -1)// la id socket
        {
           
            openingFile[index] = OpenS(flagsocket);
        }
        else
        {
...
OpenFile *
FileSystem::OpenS(int flagsocketid) 
{
    return new OpenFile(flagsocketid);
}
...
bool FileSystem::CloseSocketinfile(int indexsocketID)
 {
      openingFile[indexsocketID]=NULL;
        if (openingFile[indexsocketID]== NULL) 
        {          
            return 1;
        }
